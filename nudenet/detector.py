...
def _postprocess(
    ...
    for i in range(rows):
        ...
        if max_score >= 0.1:  # Lowered from 0.2
    ...
    indices = cv2.dnn.NMSBoxes(boxes, scores, 0.1, 0.45)  # Lowered score_threshold from 0.25
...
class NudeDetector:
    def __init__(self, model_path=None, providers=None, inference_resolution=320):
        default_model_path = os.path.join(os.path.dirname(__file__), "320n.onnx")
